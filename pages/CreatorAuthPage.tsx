...
const CreatorAuthPage: React.FC<CreatorAuthPageProps> = ({ 
  ...
  const inputBaseClasses = "w-full py-2.5 pl-10 pr-3 bg-white border rounded-lg shadow-sm focus:ring-2 focus:ring-hegira-turquoise/20 focus:border-hegira-turquoise/50 transition-colors placeholder-gray-400 text-sm";


  return (
    <div className="min-h-screen flex bg-hegira-auth-background">
      <div className="hidden lg:flex lg:w-2/5 bg-hegira-white flex-col items-center justify-center p-12 text-hegira-navy relative overflow-hidden">
          ...
          <div className="space-y-4">
            {benefits.map((benefit, index) => (
              <div key={index} className="flex items-start">
                <benefit.icon className="h-6 w-6 text-hegira-turquoise mr-3 mt-0.5 flex-shrink-0" />
      ...
      <div className="w-full lg:w-3/5 flex items-center justify-center p-6 sm:p-8 md:p-12 relative">
        <button 
          ...
          className="absolute top-6 right-6 text-xs text-hegira-auth-text/70 hover:text-hegira-auth-text inline-flex items-center"
        >
          <ArrowLeft size={14} className="mr-1" /> Kembali ke Halaman Utama
        </button>

        <div className="w-full max-w-md bg-hegira-white p-8 sm:p-10 rounded-xl shadow-2xl">
          <div className="text-center lg:hidden mb-6">
             <Logo className="h-10 mb-4 mx-auto" />
          </div>
          
          <h1 className="text-2xl sm:text-3xl font-bold text-hegira-navy text-center mb-6">
            {panelTitle}
          </h1>
          ...
          <form onSubmit={handleFormSubmit} className="space-y-5">
            ...
            {mode === 'login' && (
              <div className="text-right">
                <a href="#" className="text-xs font-medium text-hegira-auth-primary hover:text-hegira-auth-primary/80">Lupa password?</a>
              </div>
            )}
          ...
          <div className="mt-6">
            ...
            <div className="mt-4">
              <button type="button" className="w-full inline-flex justify-center items-center py-2.5 px-4 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-hegira-turquoise/20 focus:border-hegira-turquoise/50">
          ...
          <p className="mt-8 text-center text-xs text-gray-500">
            ...
            <button onClick={() => setMode(mode === 'login' ? 'signup' : 'login')} className="font-medium text-hegira-auth-primary hover:text-hegira-auth-primary/80 ml-1">
