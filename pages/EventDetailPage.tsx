  ...
  return (
    <div 
      className={`bg-white p-4 rounded-lg border transition-all duration-300 ${
        quantity > 0 ? 'border-hegira-turquoise/40' : 'border-hegira-navy/10'
    ...
    >
      <div>
        <div className="flex justify-between items-start mb-4"> {/* Gap: Title to Date/Time = 16px (was mb-6) */}
          <h4 className="text-lg font-jakarta font-semibold text-hegira-navy">{category.name}</h4>
          {currentStatus && (
        ...
        <div className="flex flex-wrap items-center text-xs text-gray-500 mb-2 gap-x-4 sm:gap-x-6 gap-y-1"> {/* Gap: Date/Time to Description = 8px (was mb-3) */}
          <div className="flex items-center">
            <CalendarDays size={14} className="mr-1.5 text-hegira-turquoise flex-shrink-0" />
            <span>{formatDisplayDate(eventDateDisplay)}</span>
          </div>
          <div className="flex items-center">
            <Clock size={14} className="mr-1.5 text-hegira-turquoise flex-shrink-0" />
      ...
      <div className="flex items-center justify-between mt-auto pt-2"> {/* Ensure this section is at the bottom */}
        <p className="text-2xl font-bold text-hegira-turquoise">{formatCurrency(category.price)}</p>
...
const EventDetailPage: React.FC<EventDetailPageProps> = ({ event, onNavigate, onNavigateRequestWithConfirmation }) => {
  ...
  if (event.name === "AI FUTURE FEST - Menuju Indonesia 5.0") {
    ...
    descriptionHtml = `
      ...
      <h3 class="text-lg font-jakarta font-semibold text-hegira-navy mt-6 mb-3">Apa yang Bisa Anda Dapatkan?</h3>
      ...
      <div class="mt-8 pt-6 border-t border-gray-200">
        <h3 class="text-lg font-jakarta font-semibold text-hegira-navy mb-3">Daftar & Informasi Kontak</h3>
        <p class="flex items-center text-gray-700 mb-1.5"><span class="text-hegira-turquoise mr-2"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.72"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.72-1.72"></path></svg></span>Daftar sekarang di: <a href="https://www.aifuturefest.com" target="_blank" rel="noopener noreferrer" class="text-hegira-turquoise hover:underline ml-1">www.aifuturefest.com</a></p>
        <p class="flex items-center text-gray-700"><span class="text-hegira-turquoise mr-2"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg></span>Kontak: (+62) 812-3456-7890 | <span class="text-hegira-turquoise mr-1 ml-1"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg></span><a href="mailto:info@aifuturefest.com" class="text-hegira-turquoise hover:underline">info@aifuturefest.com</a></p>
      </div>
    `;
  } else {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="lg:flex lg:gap-8">
          ...
          <div className="lg:w-2/3">
            ...
            <section aria-labelledby="event-main-info" className="bg-white p-6 rounded-xl border border-hegira-navy/10 mb-8 -mt-16 lg:-mt-24 relative z-10">
              <h1 id="event-main-info" className="text-3xl md:text-4xl font-bold font-jakarta text-hegira-navy mb-3">{event.name}</h1>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6 text-sm">
                <div className="flex items-start">
                  <CalendarDays size={20} className="text-hegira-turquoise mr-3 mt-0.5 flex-shrink-0" />
                  <div>
                    <strong className="block text-hegira-navy">Tanggal & Waktu</strong>
                ...
                <div className="flex items-start">
                  <MapPin size={20} className="text-hegira-turquoise mr-3 mt-0.5 flex-shrink-0" />
                  <div>
                    <strong className="block text-hegira-navy">Lokasi</strong>
                    <a 
                      href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(event.googleMapsQuery || event.location)}`} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="hover:underline text-hegira-turquoise"
                    >
                      {event.location}
                    </a>
                  </div>
                ...
                {event.parkingAvailable !== undefined && event.name !== "AI FUTURE FEST - Menuju Indonesia 5.0" && (
                  <div className="flex items-start">
                    <ParkingCircle size={20} className="text-hegira-turquoise mr-3 mt-0.5 flex-shrink-0" />
                    <div><strong className="text-hegira-navy">Parkir:</strong> {event.parkingAvailable ? 'Tersedia' : 'Terbatas/Tidak Tersedia'}</div>
                  </div>
                )}
                {event.ageRestriction && event.name !== "AI FUTURE FEST - Menuju Indonesia 5.0" && (
                  <div className="flex items-start">
                    <Users size={20} className="text-hegira-turquoise mr-3 mt-0.5 flex-shrink-0" />
                    <div><strong className="text-hegira-navy">Batasan Usia:</strong> {event.ageRestriction}</div>
                  </div>
                )}
                {event.arrivalInfo && event.name !== "AI FUTURE FEST - Menuju Indonesia 5.0" && (
                  <div className="flex items-start md:col-span-2">
                    <Clock size={20} className="text-hegira-turquoise mr-3 mt-0.5 flex-shrink-0" />
                    <div><strong className="text-hegira-navy">Info Kedatangan:</strong> {event.arrivalInfo}</div>
                  </div>
                )}
              </div>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center">
                            {event.organizerLogoUrl ? (
                                ...
                                <div className="w-10 h-10 rounded-full bg-hegira-turquoise/20 text-hegira-turquoise flex items-center justify-center text-lg font-semibold mr-3">
                                    {event.organizerName ? event.organizerName.charAt(0).toUpperCase() : <Briefcase size={20}/>}
                                </div>
                            )}
                            <span className="text-sm font-medium text-hegira-navy">{event.organizerName || 'Penyelenggara Event'}</span>
                        </div>
              ...
              <div className="relative w-full">
                <button 
                  ...
                  className="w-full flex items-center justify-center gap-2 text-hegira-turquoise hover:text-hegira-navy font-semibold py-2.5 px-4 border border-hegira-turquoise rounded-lg transition-colors"
            ...
            <section aria-labelledby="event-description-heading" className="bg-white p-6 rounded-xl border border-hegira-navy/10 mb-8">
              <div className="flex justify-between items-center mb-4">
                <h2 id="event-description-heading" className="text-2xl font-jakarta font-bold text-hegira-navy">Deskripsi Event</h2>
                {event.name !== "AI FUTURE FEST - Menuju Indonesia 5.0" && (
                  <button
                    onClick={() => setIsDescriptionExpanded(!isDescriptionExpanded)}
                    className="flex items-center text-sm text-hegira-turquoise hover:text-hegira-navy font-semibold transition-colors"
            ...
            {/* Ticket Section */}
            {event.name !== "AI FUTURE FEST - Menuju Indonesia 5.0" && (
              <section id="ticket-section" aria-labelledby="ticket-options-heading" className="bg-white p-6 rounded-xl border border-hegira-navy/10">
                <div className="flex items-center gap-3 mb-6">
                  <Ticket size={28} className="text-hegira-turquoise" />
                  <h2 id="ticket-options-heading" className="text-2xl font-jakarta font-bold text-hegira-navy">Pilih Tiket Anda</h2>
                </div>
          ...
          {event.name !== "AI FUTURE FEST - Menuju Indonesia 5.0" && (
            <div className="hidden lg:block lg:w-1/3 mt-8 lg:mt-0">
              <div className="sticky top-24 bg-white p-6 rounded-xl border border-hegira-navy/10">
                <h3 className="text-xl font-jakarta font-bold text-hegira-navy mb-4">Ringkasan Pesanan</h3>
                {totalSelectedQuantity > 0 ? (
                  <div className="space-y-2 mb-4 text-sm">
                    ...
                    <div className="border-t pt-2 mt-2 flex justify-between items-center">
                      <span className="text-base font-semibold text-hegira-navy">Total</span>
                      <span className="text-xl font-bold text-hegira-yellow">{formatCurrency(totalPrice)}</span>
                    </div>
                  </div>
                ) : (
                ...
                <button 
                  ...
                  className="w-full bg-hegira-yellow text-hegira-navy font-bold py-3 px-4 rounded-lg hover:bg-opacity-90 transition-colors flex items-center justify-center gap-2"
        ...
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white p-4 border-t border-gray-200 shadow-lg">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-xl font-bold text-hegira-yellow">{formatCurrency(totalPrice)}</p>
            </div>
            <button
              onClick={handlePrimaryAction}
              className="bg-hegira-yellow text-hegira-navy font-bold py-3 px-6 rounded-lg hover:bg-opacity-90 transition-colors flex items-center justify-center gap-2 text-sm"
              disabled={totalSelectedQuantity > 0 && event.ticketCategories.some(cat => cat.availabilityStatus === 'sold-out' && (selectedTickets[cat.id] || 0) > 0)}
            >
              {buttonIcon}
              {buttonText}
