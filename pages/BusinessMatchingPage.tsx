...
const BusinessMatchingPage: React.FC<BusinessMatchingPageProps> = ({ onNavigate }) => {
    ...
    <div className={`space-y-5 ${forMobile ? 'p-4' : ''}`}>
      {forMobile && (
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => setIsMobileFilterOpen(false)} className="text-gray-500 hover:text-hegira-turquoise">
            <X size={24} />
          </button>
        </div>
      )}
      {!forMobile && (
        <h2 className="text-xl font-semibold font-jakarta text-hegira-deep-navy mb-5 flex items-center">
          <SlidersHorizontal size={22} className="mr-2.5 text-hegira-turquoise" /> Filter Cerdas
        </h2>
      )}
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input type="text" id={`search-partner-${forMobile ? 'mobile' : 'sidebar'}`} placeholder="Nama, Lokasi, Sektor..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-hegira-turquoise/20 focus:border-hegira-turquoise/50 bg-white"/>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {industrialSectors.map(sector => (
            <button key={sector} onClick={() => handleSectorToggle(sector)} className={`px-2.5 py-1 text-xs rounded-full border transition-colors ${selectedSectors.includes(sector) ? 'bg-hegira-turquoise text-white border-hegira-turquoise' : 'bg-gray-100 text-gray-700 border-gray-300 hover:bg-gray-200'}`}>
              {sector}
            </button>
          ))}
        </div>
      ...
      <div>
        ...
        {specialFeaturesOptions.map(feature => (
            <label key={feature} className="flex items-center space-x-2 text-xs text-gray-600 cursor-pointer">
                <input type="checkbox" checked={selectedSpecialFeatures.includes(feature)} onChange={() => handleFeatureToggle(feature)} className="form-checkbox h-3.5 w-3.5 text-hegira-turquoise rounded border-gray-300 focus:ring-hegira-turquoise/20"/>
                <span>{feature}</span>
            </label>
        ))}
        </div>
      ...
      <div>
        <label htmlFor={`location-search-${forMobile ? 'mobile' : 'sidebar'}`} className="block text-sm font-medium text-gray-700 mb-1">Cari Lokasi di Peta</label>
         <div className="relative">
             ...
             <input type="text" id={`location-search-${forMobile ? 'mobile' : 'sidebar'}`} placeholder="cth: Jakarta Pusat" value={selectedLocation} onChange={(e) => setSelectedLocation(e.target.value)} className="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-hegira-turquoise/20 focus:border-hegira-turquoise/50 bg-white" />
        </div>
      </div>
      <div>
         <label htmlFor={`radiusFilter-${forMobile ? 'mobile' : 'sidebar'}`} className="block text-sm font-medium text-gray-700 mb-1">Radius Filter: <span className="font-bold text-hegira-turquoise">{radiusFilter} km</span></label>
         <input type="range" id={`radiusFilter-${forMobile ? 'mobile' : 'sidebar'}`} min="5" max="200" step="5" value={radiusFilter} onChange={(e) => setRadiusFilter(parseInt(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-hegira-turquoise focus:outline-none focus:ring-2 focus:ring-hegira-turquoise/20" />
      </div>

      <div className="pt-4 border-t border-gray-200 flex flex-col gap-2">
        <button onClick={applyFiltersAction} className="w-full bg-hegira-yellow text-hegira-navy font-semibold py-2 px-4 rounded-lg hover:bg-opacity-90 transition-colors text-sm">Terapkan Filter</button>
        <button onClick={resetFilters} className="w-full text-gray-600 hover:text-hegira-turquoise font-medium py-2 px-4 rounded-lg border border-gray-300 hover:border-gray-400 transition-colors text-sm flex items-center justify-center gap-1.5">
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero Banner */}
      <div className="w-full bg-gradient-to-r from-hegira-turquoise/10 via-hegira-yellow/10 to-hegira-turquoise/10 relative" style={{ paddingTop: '37.5%' /* 16:6 aspect ratio */ }}>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        <header className="mb-6 md:mb-8 text-center lg:text-left flex flex-col lg:flex-row justify-between items-center">
          <div>
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold font-jakarta text-hegira-deep-navy">
          ...
          <button 
            className="lg:hidden mt-4 bg-hegira-turquoise text-white px-4 py-2 rounded-lg shadow hover:bg-opacity-90 flex items-center gap-2"
            ...
            {totalPages > 1 && (
              <nav aria-label="Paginasi partner" className="mt-10 md:mt-12 flex justify-center">
                <ul className="inline-flex items-center -space-x-px shadow-sm">
                  <li><button onClick={() => handlePageChange(Math.max(1, currentPage - 1))} disabled={currentPage === 1} className="pagination-arrow rounded-l-lg focus:ring-2 focus:ring-hegira-turquoise/20 focus:border-hegira-turquoise/50"><ChevronLeft size={18}/></button></li>
                  {Array.from({length: totalPages}, (_, i) => i + 1).map(num => (
                    <li key={num}><button onClick={() => handlePageChange(num)} className={`pagination-number ${currentPage === num ? 'active' : ''} focus:ring-2 focus:ring-hegira-turquoise/20 focus:border-hegira-turquoise/50`}>{num}</button></li>
                  ))}
                  <li><button onClick={() => handlePageChange(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className="pagination-arrow rounded-r-lg focus:ring-2 focus:ring-hegira-turquoise/20 focus:border-hegira-turquoise/50"><ChevronRight size={18}/></button></li>
                </ul>
              </nav>
            )}
          </main>
      ...
      <style>{`
        ...
        .pagination-arrow:hover { background-color: #f9fafb; color: var(--hegira-turquoise); }
        ...
        .pagination-number:hover { background-color: #f9fafb; color: var(--hegira-turquoise); }
        .pagination-number.active { color: white; background-color: var(--hegira-turquoise); border-color: var(--hegira-turquoise); font-weight: 600; z-index: 10; }
      `}</style>
    </div>
  );
};
