    ...
    return (
      <div className="mt-8 flex flex-col sm:flex-row justify-between items-center text-sm text-gray-600 bg-white p-4 rounded-b-lg border-t border-gray-200">
        ...
        <div className="flex items-center space-x-1">
          ...
          {pageNumbers.map((number, index) => typeof number === 'string' ? <span key={`ellipsis-${index}`} className="px-2 py-1.5 text-gray-500">...</span> : <button key={number} onClick={() => setCurrentPage(number)} className={`px-3 py-1.5 rounded-md transition-colors border ${currentPage === number ? 'bg-hegira-turquoise text-white font-semibold border-hegira-turquoise' : 'bg-white text-gray-700 hover:bg-gray-100 border-gray-300'}`}>{number}</button>)}
  ...
  return (
    <div className="p-0 sm:p-2 md:p-4 bg-gray-50 min-h-full space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-hegira-deep-navy">Data Pengunjung</h1>
          <p className="text-sm text-gray-600 mt-1">Menampilkan pengunjung untuk event: <strong className="text-hegira-turquoise">{selectedEvent.name}</strong></p>
        </div>
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-6" aria-label="Tabs">
          {(['data', 'laporan', 'checkin'] as const).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm ${activeTab === tab ? 'border-hegira-turquoise text-hegira-turquoise' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
      ...
      {activeTab === 'data' && (
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <h2 className="text-xl font-semibold text-hegira-deep-navy mb-4">Data Pengunjung untuk "{selectedEvent.name}"</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 xl:grid-cols-6 gap-4 items-end mb-6">
            <div className="lg:col-span-2">
              ...
              <div className="relative"><Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" /><input type="text" id="search-attendee" placeholder="ID, Nama, Telepon..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-hegira-turquoise focus:border-hegira-turquoise bg-white" /></div>
            </div>
            <div><label htmlFor="filter-ticket-cat" className="block text-xs font-medium text-gray-600 mb-1">Kategori Tiket</label><select id="filter-ticket-cat" value={filterTicketCategory} onChange={e => setFilterTicketCategory(e.target.value)} className="w-full py-2 px-3 text-sm border border-gray-300 rounded-lg focus:ring-hegira-turquoise focus:border-hegira-turquoise appearance-none bg-white">{uniqueTicketCategories.map(cat => <option key={cat} value={cat}>{cat}</option>)}</select></div>
            <div><label htmlFor="filter-status-att" className="block text-xs font-medium text-gray-600 mb-1">Status Kehadiran</label><select id="filter-status-att" value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="w-full py-2 px-3 text-sm border border-gray-300 rounded-lg focus:ring-hegira-turquoise focus:border-hegira-turquoise appearance-none bg-white">{statusOptions.map(stat => <option key={stat} value={stat}>{stat}</option>)}</select></div>
            <div><label htmlFor="sort-order-att" className="block text-xs font-medium text-gray-600 mb-1">Urutkan</label><select id="sort-order-att" value={sortOrder} onChange={e => setSortOrder(e.target.value as typeof sortOrder)} className="w-full py-2 px-3 text-sm border border-gray-300 rounded-lg focus:ring-hegira-turquoise focus:border-hegira-turquoise appearance-none bg-white"><option value="eventDate_desc">Tanggal Event (Terbaru)</option><option value="eventDate_asc">Tanggal Event (Terlama)</option><option value="ownerName_asc">Nama Pemilik (A-Z)</option><option value="ownerName_desc">Nama Pemilik (Z-A)</option><option value="status_asc">Status (A-Z)</option><option value="status_desc">Status (Z-A)</option></select></div>
                ...
                {currentDisplayAttendees.length > 0 ? currentDisplayAttendees.map(att => (
                  <tr key={att.ticketId} className={`hover:bg-gray-50 transition-colors ${att.status === 'Sudah Hadir' ? 'bg-green-50' : ''}`}>
                    ...
                    <td className="px-4 py-3 text-center whitespace-nowrap">
                      <button onClick={() => handleOpenEditStatusModal(att)} className="text-gray-400 hover:text-hegira-turquoise p-1" aria-label={`Edit status untuk ${att.ownerName}`}>
                        <Edit3 size={14} />
                      </button>
                    </td>
                  </tr>
