...
const DetailRightColumnItem: React.FC<{ label: string; value?: string | number | null; valueClassName?: string; isLink?: boolean; href?: string; }> = ({ label, value, valueClassName = "text-hegira-deep-navy text-right", isLink, href }) => {
    ...
    <div className="pb-8">
      ...
      <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
        ...
        <div className="p-4 sm:p-6 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <h1 id="event-detail-page-title" className="text-xl md:text-2xl font-bold text-hegira-deep-navy leading-tight">
              {eventData.name}
            </h1>
          ...
          <div className="w-full lg:w-3/5 order-2 lg:order-1 p-4 sm:p-6 lg:pr-3 xl:pr-6">
            ...
            <section className="mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={handleCopyLink} title="Salin Link" className="p-2.5 text-gray-600 hover:text-hegira-turquoise bg-gray-100 hover:bg-gray-200 rounded-md border border-gray-300"><Upload size={18} /></button>
                <button onClick={handlePreviewLink} title="Pratinjau Link" className="p-2.5 text-gray-600 hover:text-hegira-turquoise bg-gray-100 hover:bg-gray-200 rounded-md border border-gray-300"><Eye size={18} /></button>
              </div>
            ...
            <section className="mb-6">
              <div className="flex justify-between items-center mb-1.5">
                ...
                <button onClick={() => setIsDescriptionExpanded(!isDescriptionExpanded)} className="text-xs text-hegira-turquoise hover:underline flex items-center">
            ...
            <section>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <button onClick={() => setIsTermsExpanded(!isTermsExpanded)} className="text-xs text-hegira-turquoise hover:underline flex items-center">
          ...
          <div className="w-full lg:w-2/5 order-1 lg:order-2 p-4 sm:p-6 lg:border-l lg:border-gray-200">
            <section className="mb-5">
              ...
              <div className="space-y-1 text-sm">
                <DetailRightColumnItem label="Kategori" value={eventData.category} valueClassName="text-hegira-deep-navy font-medium text-right"/>
                <DetailRightColumnItem label="Tema" value={eventData.theme} valueClassName="text-hegira-deep-navy font-medium text-right"/>
                <DetailRightColumnItem label="Tanggal" value={formatDateRange(eventData.dateDisplay)} valueClassName="text-hegira-deep-navy text-right"/>
                <DetailRightColumnItem label="Waktu" value={formatEventTime(eventData.timeDisplay, eventData.timezone)} valueClassName="text-hegira-deep-navy text-right"/>
                <DetailRightColumnItem label="Lokasi" value={eventData.location} valueClassName="text-hegira-deep-navy text-right"/>
                <DetailRightColumnItem label="Alamat" value={eventData.address} valueClassName="text-hegira-deep-navy text-right"/>
                <DetailRightColumnItem label="Google Maps" value={eventData.googleMapsQuery || "Belum diatur"} isLink={!!eventData.googleMapsQuery} href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(eventData.googleMapsQuery || eventData.location)}`} valueClassName="text-hegira-turquoise text-right"/>
              </div>
            </section>
