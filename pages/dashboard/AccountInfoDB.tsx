...
const AccountInfoDB: React.FC<AccountInfoDBProps> = ({ creatorData, onUpdatePhoneNumber, onUpdateProfilePicture, onUpdatePassword }) => {
    ...
    <div>
        ...
        <div className="relative">
            ...
            <input 
                ...
                required={!disabled}
                disabled={disabled}
                className={`w-full py-2.5 pl-10 pr-10 border rounded-lg shadow-sm focus:ring-1 focus:ring-hegira-turquoise transition-colors text-sm ${error ? 'border-red-500' : 'border-gray-300'} ${disabled ? 'bg-white text-gray-500 cursor-not-allowed' : 'bg-white'}`}
                placeholder={label}
            />
            <button type="button" onClick={showToggler} className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-500 hover:text-hegira-navy" disabled={disabled}>
  ...
  return (
    <div className="p-0 sm:p-2 md:p-4 bg-gray-50 min-h-full">
      <h1 className="text-2xl sm:text-3xl font-bold text-hegira-deep-navy mb-8">Informasi Akun Saya</h1>
        ...
        <div className="text-center mb-8">
          <div className="relative w-36 h-36 rounded-full mx-auto mb-3 group">
            <img
              ...
              alt="Foto Profil"
              className="w-full h-full rounded-full object-cover border-4 border-hegira-turquoise/30"
            />
            <button 
          ...
          <h2 className="text-xl font-semibold text-hegira-navy mt-3">{creatorData.fullName}</h2>
        ...
        <div className="space-y-6 max-w-2xl mx-auto">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-shrink-0">
                 <select 
                  ...
                  className="appearance-none z-10 h-full block w-auto py-2.5 pl-3 pr-8 text-sm border rounded-l-lg focus:ring-hegira-turquoise focus:border-hegira-turquoise bg-white disabled:bg-white disabled:text-gray-500 disabled:cursor-not-allowed border-gray-300"
                >
              ...
              <div className="relative flex-grow">
                ...
                <input
                  ...
                  className="w-full py-2.5 px-3 border border-gray-300 rounded-r-lg shadow-sm focus:ring-hegira-turquoise focus:border-hegira-turquoise text-sm disabled:bg-white disabled:text-gray-500 disabled:cursor-not-allowed bg-white"
                />
              </div>
              {!isPhoneNumberDisabled && (
                <button 
                  onClick={handleSavePhoneNumber}
                  className="p-2.5 bg-hegira-turquoise text-white rounded-lg hover:bg-opacity-90 transition-colors flex-shrink-0"
            ...
            <div className="mt-2 flex items-center">
              <input type="checkbox" id="isWhatsappActive" name="isWhatsappActive" defaultChecked className="h-4 w-4 text-hegira-turquoise border-gray-300 rounded focus:ring-hegira-turquoise cursor-not-allowed" disabled />
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
                <div className="relative flex-grow">
                    ...
                    />
                    <button type="button" onClick={() => setShowCurrentPassword(!showCurrentPassword)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-500 hover:text-hegira-navy">
          ...
          {showPasswordForm && (
            <form onSubmit={handlePasswordSubmit} className="space-y-4 mt-4 pt-4 border-t border-gray-100">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                  <button type="submit" className="w-full sm:w-auto flex items-center justify-center gap-2 bg-hegira-yellow text-hegira-navy font-bold py-2.5 px-4 rounded-lg hover:bg-opacity-90 transition-colors text-sm">
      ...
      {showConfirmModal && (
        <ConfirmationModal
          ...
          confirmButtonClass="bg-hegira-turquoise hover:bg-hegira-turquoise/90 text-white"
        />
      )}
    </div>
  );
