    ...
    <div className="p-0 sm:p-2 md:p-4 bg-gray-50 min-h-full">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
            <div className="flex-grow">
                <h1 className="text-2xl sm:text-3xl font-bold text-hegira-deep-navy">Manajemen Crew Event</h1>
                <p className="text-sm text-gray-600 mt-1">Event: <strong className="text-hegira-turquoise">{selectedEvent.name}</strong></p>
            </div>
            <button
            onClick={() => setShowCrewIDCardModal(true)}
            className="bg-hegira-yellow text-hegira-navy font-semibold py-2 px-4 rounded-lg hover:bg-opacity-90 transition-colors flex items-center gap-2 text-sm mt-3 sm:mt-0"
            >
      ...
      <div className="bg-white p-4 rounded-lg border border-gray-200 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
          ...
          <div>
            ...
            <select id="filter-role-crew" value={filterRole} onChange={e => setFilterRole(e.target.value)} className="w-full py-2 px-3 text-sm border border-gray-300 rounded-lg focus:ring-hegira-turquoise focus:border-hegira-turquoise appearance-none bg-white">{uniqueRoles.map(role => <option key={role} value={role}>{role}</option>)}</select>
          </div>
          <div>
            <label htmlFor="filter-status-crew" className="block text-xs font-medium text-gray-600 mb-1">Filter Status</label>
            <select id="filter-status-crew" value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="w-full py-2 px-3 text-sm border border-gray-300 rounded-lg focus:ring-hegira-turquoise focus:border-hegira-turquoise appearance-none bg-white">
          ...
          <div className="flex items-center space-x-2 md:col-span-2 lg:col-span-1">
             ...
             <button
                ...
                className="bg-hegira-turquoise hover:bg-hegira-turquoise/90 text-white font-semibold py-2.5 px-4 rounded-lg transition-colors flex items-center gap-1.5 text-sm flex-grow justify-center"
             >
            ...
            {currentDisplayCrew.length > 0 ? currentDisplayCrew.map(crew => (
              <tr key={crew.id} className={`hover:bg-gray-50 transition-colors ${crew.status === 'Hadir' ? 'bg-green-50' : ''}`}>
                ...
                <td className="px-4 py-3 text-center whitespace-nowrap">
                  <button onClick={() => handleOpenEditStatusModal(crew)} className="text-gray-400 hover:text-hegira-turquoise p-1 mx-1" title="Edit Status"><Edit3 size={16} /></button>
