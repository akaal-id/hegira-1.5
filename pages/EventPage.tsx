  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      <header className="mb-8 md:mb-12 text-center">
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-jakarta font-bold text-hegira-turquoise">Jelajahi Event</h1>
      ...
      <div className="mb-12 md:mb-16">
        <div className="relative max-w-2xl mx-auto">
          <input
            ...
            className="w-full py-3 px-4 pl-12 border border-gray-300 rounded-full shadow-sm focus:ring-2 focus:ring-hegira-turquoise/20 focus:border-hegira-turquoise/50 transition-colors bg-white" 
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <aside className="w-full md:w-1/4 lg:w-1/5 bg-white p-6 rounded-lg shadow-sm border border-gray-200 self-start">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-jakarta font-semibold text-hegira-navy">Filters</h2>
            <button 
              onClick={() => setIsFilterVisible(!isFilterVisible)} 
              className="text-hegira-navy hover:text-hegira-turquoise p-1"
              ...
              <div>
                ...
                <ul className="space-y-1">
                  {categories.map(category => (
                    <li key={category}>
                      <label className="flex items-center space-x-2 text-gray-700 hover:text-hegira-turquoise cursor-pointer">
                        <input
                          type="checkbox"
                          checked={selectedCategories.includes(category)}
                          onChange={() => handleCategoryChange(category)}
                          className="form-checkbox h-4 w-4 text-hegira-turquoise rounded border-gray-300 focus:ring-hegira-turquoise/20"
                        />
                        <span>{category}</span>
                      </label>
                    </li>
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-hegira-turquoise/20 focus:border-hegira-turquoise/50 text-sm bg-white"
          ...
          {totalPages > 1 && (
            <div className="mt-12 flex justify-center items-center space-x-2" aria-label="Paginasi event">
              <button
                ...
                className="px-3 py-1 rounded-md text-sm bg-white text-gray-700 hover:bg-gray-100 border border-gray-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center focus:ring-2 focus:ring-hegira-turquoise/20 focus:border-hegira-turquoise/50"
              ...
              {pageNumbers.map(number => (
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-sm transition-colors focus:ring-2 focus:ring-hegira-turquoise/20 focus:border-hegira-turquoise/50
                    ${currentPage === number 
                      ? 'bg-hegira-turquoise text-white font-semibold' 
