  ...
  return (
    <div className="p-0 sm:p-2 md:p-4 bg-gray-50 min-h-full">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-hegira-deep-navy">Jadwal Meeting Anda</h1>
        <button
      ...
      <div className="mb-4 flex justify-end">
        <div className="inline-flex rounded-md shadow-sm">
          <button onClick={() => setViewMode('list')} className={`px-3 py-1.5 text-xs rounded-l-md border ${viewMode === 'list' ? 'bg-hegira-turquoise text-white border-hegira-turquoise z-10' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}>Daftar</button>
          <button onClick={() => setViewMode('calendar')} className={`px-3 py-1.5 text-xs rounded-r-md border -ml-px ${viewMode === 'calendar' ? 'bg-hegira-turquoise text-white border-hegira-turquoise z-10' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}>Kalender (Segera)</button>
        </div>
      </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-2">
                        <div className="flex-grow">
                            <div className="flex items-center mb-1">
                                <MeetingIcon size={16} className="text-hegira-turquoise mr-2 flex-shrink-0" />
                                <h3 className="text-md font-semibold text-hegira-deep-navy truncate" title={meeting.title}>{meeting.title}</h3>
                            </div>
