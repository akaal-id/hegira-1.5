  ...
  const renderInfoField = (label: string, value: string | undefined | null, Icon: React.ElementType, isEditable: boolean = true, name?: keyof OrganizationAccountData, type: string = 'text', asTextarea: boolean = false) => {
      ...
      if (asTextarea) {
        return (
          <div className="mb-3">
            ...
            <textarea id={name} name={name} value={formData[name] as string || ''} onChange={handleInputChange} rows={3} className="w-full py-2 px-3 text-sm border border-gray-300 rounded-md focus:ring-hegira-turquoise focus:border-hegira-turquoise shadow-sm bg-white"></textarea>
          </div>
        );
      }
      return (
        <div className="mb-3">
          ...
          <div className="relative">
            ...
            <input type={type} id={name} name={name} value={formData[name] as string || ''} onChange={handleInputChange} className={`w-full py-2 ${Icon ? 'pl-8' : 'pl-3'} pr-3 text-sm border border-gray-300 rounded-md focus:ring-hegira-turquoise focus:border-hegira-turquoise shadow-sm bg-white`} />
          </div>
        </div>
      );
    }
    return (
      <div className="flex items-start py-1.5">
        ...
        <span className="text-sm text-hegira-deep-navy font-medium break-words flex-grow">{value || '-'}</span>
      </div>
    );
  };
  ...
  return (
    <div className="p-0 sm:p-2 md:p-4 bg-gray-50 min-h-full">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-hegira-deep-navy">Profil Perusahaan Anda</h1>
        {!isEditing && (
          <button
            onClick={() => setIsEditing(true)}
            className="bg-hegira-turquoise text-white font-semibold py-2 px-4 rounded-lg hover:bg-opacity-90 transition-colors flex items-center gap-2 text-sm mt-3 sm:mt-0"
          >
      ...
      <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
        ...
        <div className="flex flex-col items-center mb-8 text-center">
          ...
          {!isEditing && <h2 className="text-xl font-semibold text-hegira-navy">{formData.organizationName}</h2>}
        ...
        {isEditing && (
          <div className="mt-8 pt-6 border-t border-gray-200 flex flex-col sm:flex-row justify-end gap-3">
            ...
            <button onClick={handleSaveChanges} type="button" className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-bold text-white bg-hegira-turquoise hover:bg-opacity-90 rounded-lg shadow-sm transition-colors">
