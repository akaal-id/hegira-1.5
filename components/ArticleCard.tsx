...
const ArticleCard: React.FC<ArticleCardProps> = ({
  ...
  const categoryColors: { [key: string]: string } = {
    'Tips & Trik': 'bg-hegira-turquoise text-white',
    'Studi Kasus': 'bg-hegira-yellow text-hegira-navy',
    'Wawasan': 'bg-hegira-navy text-white',
  ...
  return (
    <div 
      className="bg-white rounded-xl border border-gray-200 hover:border-hegira-turquoise/30 overflow-hidden transition-all duration-300 group flex flex-col h-full shadow-sm hover:shadow-lg"
      role="article"
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow">
        <h3 
            ...
            className="text-lg font-bold font-jakarta text-hegira-deep-navy mb-2 leading-tight group-hover:text-hegira-gradient-start transition-colors cursor-pointer"
        ...
        <button
          onClick={() => onNavigate(slug)}
          className="mt-auto w-full sm:w-auto self-start bg-transparent text-hegira-turquoise font-semibold py-2 px-0 rounded-lg
                     transition-all duration-300
                     flex items-center group/button
                     hover:text-hegira-navy"
