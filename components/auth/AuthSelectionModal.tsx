...
const AuthSelectionModal: React.FC<AuthSelectionModalProps> = ({ isOpen, onClose, onSelectRole }) => {
  ...
  // Filter out the "Organization" role
      ...
      <div className="relative bg-hegira-white w-full max-w-md rounded-2xl shadow-2xl border border-hegira-navy/10 overflow-hidden transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear">
        ...
        <div className="p-6 sm:p-8">
          <div className="text-center mb-8">
            ...
            <h1 id="auth-selection-modal-title" className="text-xl sm:text-2xl font-semibold text-hegira-navy">Selamat Datang di Hegira!</h1>
            ...
            {roles.map((roleItem) => (
              <button
                ...
                className="w-full flex items-center justify-start text-left py-3 sm:py-3.5 px-4 sm:px-5 border border-gray-300 rounded-lg shadow-sm 
                           hover:border-hegira-turquoise hover:bg-hegira-turquoise/5 transition-all duration-200
                           focus:outline-none focus:ring-2 focus:ring-hegira-yellow focus:ring-offset-2"
              >
                <roleItem.icon className="h-5 w-5 sm:h-6 sm:w-6 text-hegira-turquoise mr-3 sm:mr-4 flex-shrink-0" />
                <div>
                    <span className="text-sm sm:text-base font-medium text-hegira-navy">{roleItem.label}</span>
          ...
          <div className="mt-6 text-center">
            <p className="text-xs text-gray-600">
              ...
              <a 
                ...
                className="text-hegira-yellow hover:text-yellow-400 font-semibold transition-colors inline-flex items-center"
              >
          ...
          <p className="mt-8 text-center text-xs text-gray-400">
            Dengan melanjutkan, Anda menyetujui <a href="#" className="underline hover:text-hegira-turquoise">Syarat & Ketentuan</a> kami.
          </p>
        </div>
      </div>
      <style>{`
