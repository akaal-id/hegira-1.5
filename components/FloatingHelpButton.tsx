...
const FloatingHelpButton: React.FC<FloatingHelpButtonProps> = ({ onNavigate }) => {
  ...
  const subButtons = [
    {
      ...
      bgColor: 'bg-hegira-turquoise',
      textColor: 'text-white',
    },
    {
      label: 'Email Kami',
      icon: Mail,
      action: () => { window.location.href = 'mailto:support@hegira.com?subject=Bantuan%20hegira'; setIsOpen(false); },
      bgColor: 'bg-red-500',
      textColor: 'text-white',
    },
    {
      label: 'WhatsApp Kami',
      icon: MessageCircle,
      action: () => { window.open('https://wa.me/6281234567890?text=Halo%20hegira,%20saya%20butuh%20bantuan.', '_blank'); setIsOpen(false); },
  ...
  return (
    <div ref={fabRef} className="fixed bottom-6 right-6 z-[999]">
      ...
      <button
        onClick={toggleOpen}
        className="w-16 h-16 bg-hegira-navy text-white rounded-full flex items-center justify-center
                   shadow-xl hover:bg-hegira-turquoise focus:outline-none focus:ring-2
                   focus:ring-offset-2 focus:ring-hegira-yellow transition-all duration-300 ease-in-out transform hover:scale-110"
