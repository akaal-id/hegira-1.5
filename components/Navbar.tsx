...
export const UserMenuButton: React.FC<UserMenuButtonProps> = ({ 
  ...
  const userMenuButtonActiveStyles = `bg-hegira-turquoise/10 text-hegira-turquoise border border-hegira-turquoise hover:bg-hegira-turquoise/20`;
  const userMenuButtonInactiveStyles = `text-hegira-deep-navy hover:bg-hegira-turquoise/5`;
    ...
    <div ref={userMenuRef} className="relative w-full sm:w-auto">
      ...
      >
        <div className="flex items-center gap-2">
          <UserCircle size={20} className={isUserMenuOpen ? "text-hegira-turquoise" : "text-hegira-deep-navy group-hover:text-hegira-turquoise"} />
          <span className={`truncate max-w-[100px] sm:max-w-[120px] ${isUserMenuOpen ? "text-hegira-turquoise" : "text-hegira-deep-navy group-hover:text-hegira-turquoise"}`}>Hi, {userName}!</span>
        </div>
        <ChevronDown size={16} className={`transition-transform duration-200 ${isUserMenuOpen ? 'rotate-180' : ''} ${isUserMenuOpen ? "text-hegira-turquoise" : "text-hegira-deep-navy/70 group-hover:text-hegira-turquoise"}`} />
      </button>
        ...
        >
          <div className="py-1" role="none">
            <button
              ...
              className="w-full text-left flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-hegira-turquoise/10 hover:text-hegira-turquoise transition-colors"
              role="menuitem"
            >
...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  const navbarBgColor = 'bg-hegira-white'; 
  const navbarBorderColor = 'border-b border-hegira-turquoise/10';
  const logoTextColor = 'text-hegira-turquoise'; 
  
  const mobileIconColor = `text-hegira-deep-navy hover:bg-gray-100`; 
  const mobileMenuBorderColor = 'border-hegira-chino';

  const authButtonStyles = `bg-hegira-turquoise/10 text-hegira-turquoise border border-hegira-turquoise px-5 py-2.5 rounded-full text-sm font-semibold 
                            hover:bg-hegira-turquoise/20 transition-colors flex items-center justify-center gap-2`;
  
  // Styles for the "Buat Event" button when user is NOT logged in
  const createEventNotLoggedInStyles = `bg-hegira-yellow text-hegira-navy border border-hegira-yellow px-5 py-2.5 rounded-full text-sm font-semibold hover:bg-opacity-90 transition-colors flex items-center justify-center gap-2 transform hover:scale-105`;
  const createEventNotLoggedInMobileStyles = `w-full text-center bg-hegira-yellow text-hegira-navy border border-hegira-yellow px-5 py-3 rounded-full text-sm font-semibold hover:bg-opacity-90 transition-colors flex items-center justify-center gap-2`;
  ...
  if (isLoggedIn) {
    ...
    switch (userRole) {
      case 'visitor':
        ...
        dynamicButtonStyles = `bg-hegira-turquoise/10 text-hegira-turquoise border border-hegira-turquoise hover:bg-hegira-turquoise/20 ${baseModeButtonStyles}`;
        break;
      case 'creator':
        dynamicButtonText = "Kreator";
        dynamicButtonStyles = `bg-hegira-yellow/10 text-hegira-navy border border-hegira-yellow hover:bg-hegira-yellow/20 ${baseModeButtonStyles}`;
        break;
      case 'organization':
        dynamicButtonText = "Organisasi";
        dynamicButtonStyles = `bg-hegira-chino/10 text-hegira-navy border border-hegira-chino hover:bg-hegira-chino/20 ${baseModeButtonStyles}`;
        break;
      default: 
              ...
              <button
                ...
                className={`${desktopNavLinkBaseClasses} ${
                  (currentPage === item.target)
                    ? 'font-semibold text-hegira-turquoise bg-hegira-turquoise/10 rounded-full'
                    : 'font-medium text-hegira-deep-navy hover:font-semibold hover:text-hegira-turquoise'
                }`}
          ...
          <div className="hidden md:flex items-center space-x-3">
             {isLoggedIn ? ( 
                ...
                <>
                  ...
                  >
                      <UserCircle size={18} className="text-hegira-turquoise" /> Masuk / Daftar
                  </button>
                </>
             )}
          </div>
      ...
      {isMobileMenuOpen && (
        <div className={`md:hidden absolute top-0 inset-x-0 min-h-screen ${navbarBgColor}/95 backdrop-blur-lg p-4 pt-20 space-y-2 sm:px-6 z-[-1] flex flex-col animate-fade-in`} id="mobile-menu">
          ...
          {isLoggedIn && ( /* This is the dynamic role button for mobile when logged in */
            <button 
                onClick={handleRoleSpecificBuatEventClick}
                className={`${mobileNavLinkBaseClasses} font-medium text-hegira-deep-navy hover:font-semibold hover:text-hegira-turquoise flex items-center gap-2`}
                aria-label={dynamicButtonAriaLabel}
            >
                <DynamicButtonIcon size={18} /> {dynamicButtonText}
            </button>
