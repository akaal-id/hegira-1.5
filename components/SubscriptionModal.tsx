...
const SubscriptionModal: React.FC<SubscriptionModalProps> = ({ isOpen, onClose, onSubscribe }) => {
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="relative bg-gradient-to-br from-hegira-navy via-hegira-navy to-hegira-turquoise/80 text-hegira-white w-full max-w-lg rounded-2xl shadow-2xl border border-hegira-yellow/30 overflow-hidden transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-hegira-yellow/70 hover:text-hegira-yellow transition-colors z-20"
        ...
        <div className="p-6 sm:p-8">
          <div className="text-center mb-6">
            <Briefcase size={48} className="mx-auto mb-4 text-hegira-yellow" />
            <h1 id="subscription-modal-title" className="text-2xl sm:text-3xl font-bold text-white">
              Akses Eksklusif <span className="text-hegira-yellow">Business Matching</span>
            </h1>
          ...
          <div className="bg-white/10 p-4 rounded-lg mb-6">
            <h2 className="text-lg font-semibold text-hegira-yellow mb-2 text-center sm:text-left">Keuntungan Berlangganan:</h2>
            <ul className="space-y-2 text-sm">
              {benefits.map((benefit, index) => (
                <li key={index} className="flex items-start">
                  <CheckCircle size={18} className="text-hegira-yellow mr-2 mt-0.5 flex-shrink-0" />
          ...
          <div className="space-y-3">
            ...
            <div className="relative w-full rounded-lg overflow-hidden"> 
                ...
                className="w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-lg shadow-md text-base font-bold
                            bg-hegira-yellow text-hegira-navy hover:bg-opacity-90 
                            focus:outline-none focus:ring-2 focus:ring-hegira-navy focus:ring-offset-hegira-navy
            ...
            <button
              onClick={onClose}
              type="button"
              className="w-full flex justify-center items-center py-3 px-4 border border-hegira-yellow/50 rounded-lg shadow-sm text-base font-medium
                         text-hegira-yellow hover:bg-hegira-yellow/10
                         focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-hegira-navy focus:ring-white
                         transition-colors"
            >
              Batal
            </button>
