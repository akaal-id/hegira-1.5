...
const BusinessMatchingCard: React.FC<BusinessMatchingCardProps> = (props) => {
  ...
  return (
    <div 
      className="bg-hegira-white rounded-xl border border-hegira-navy/10 hover:border-hegira-turquoise/30 overflow-hidden transition-all duration-300 group flex flex-col h-full shadow-sm hover:shadow-lg cursor-pointer"
    ...
    >
      <div className="p-5 sm:p-6 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-4">
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden border-2 border-hegira-turquoise/20 flex-shrink-0 bg-gray-100 flex items-center justify-center">
            <img
              src={logoUrl}
              alt={`Logo ${name}`}
          ...
          <button
            ...
            className="p-2 text-hegira-yellow hover:text-hegira-yellow/80 focus:outline-none"
        ...
        <h3 className="text-xl sm:text-2xl font-bold font-jakarta text-hegira-deep-navy group-hover:text-hegira-turquoise transition-colors mb-2 truncate" title={name}>
          {name}
        </h3>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {badgesToDisplay.map((feature, index) => (
            <span 
              key={index} 
              className="text-[10px] sm:text-xs bg-hegira-turquoise/10 text-hegira-turquoise font-semibold px-2.5 py-1 rounded-full flex items-center gap-1 shadow-sm"
            >
        ...
        <div className="space-y-2 text-sm text-gray-600 mb-5 flex-grow">
          <div className="flex items-center">
            <Briefcase size={15} className="mr-2.5 text-hegira-turquoise/80 flex-shrink-0" />
            <span>Sektor: <strong className="text-hegira-deep-navy/90">{sector}</strong></span>
          </div>
          <div className="flex items-center">
            <MapPin size={15} className="mr-2.5 text-hegira-turquoise/80 flex-shrink-0" />
            <span className="truncate" title={location}>Lokasi: <strong className="text-hegira-deep-navy/90">{location}</strong></span>
          </div>
          <div className="flex items-center">
            <DollarSign size={15} className="mr-2.5 text-hegira-turquoise/80 flex-shrink-0" />
            <span>Budget: <strong className="text-hegira-deep-navy/90">{budget}</strong></span>
          </div>
        </div>

        <div className="mt-auto pt-4 border-t border-gray-100">
          <button // This button is part of the card's main click area due to onClick on parent div
            className="w-full text-hegira-turquoise font-semibold py-2.5 px-4 rounded-lg
                       transition-all duration-300
                       flex items-center justify-center group/button
                       hover:bg-hegira-turquoise/10"
