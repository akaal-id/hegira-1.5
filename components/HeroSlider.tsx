...
const HeroSlider: React.FC<HeroSliderProps> = ({ events, onNavigate }) => {
    ...
    return (
      ...
      >
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative text-hegira-white overflow-hidden rounded-3xl border border-hegira-navy/10">
  ...
  const renderSlideContent = (event: EventData) => {
    ...
    return (
      <div 
        className="w-full h-full relative cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-hegira-turquoise focus-visible:ring-offset-2 rounded-lg"
                  ...
                  {events.map((_, index) => (
                    <button
                      ...
                      className={`w-2.5 h-2.5 md:w-3 md:h-3 rounded-full transition-all duration-300 ease-in-out
                                  ${currentSlide === index ? 'bg-hegira-turquoise scale-125' : 'bg-white/60 hover:bg-white/90'}`}
