  ...
  return (
    ...
    >
      <div className="bg-white text-hegira-navy p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-md relative transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear">
        ...
        <h2 id="edit-status-modal-title" className="text-xl font-semibold text-hegira-navy mb-2">
          Ubah Status Pesanan
        </h2>
        <p className="text-sm text-gray-500 mb-6">
          Untuk Pesanan ID: <span className="font-medium text-hegira-turquoise">{orderId}</span>
        </p>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center text-left p-3 border rounded-lg transition-colors
                                  ${isCurrent 
                                    ? `bg-hegira-turquoise/10 border-hegira-turquoise/50 ${color}` 
        ...
        <div className="mt-8 flex justify-end">
          <button
            onClick={onClose}
            type="button"
            className="inline-flex justify-center rounded-lg border border-gray-300 shadow-sm px-4 py-2 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-hegira-turquoise transition-colors"
          >
            Batal
          </button>
        </div>
