...
const AddTicketModal: React.FC<AddTicketModalProps> = ({ 
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="bg-white text-hegira-navy p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-lg relative transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear max-h-[90vh] overflow-y-auto custom-scrollbar-modal">
        <div className="flex justify-between items-center mb-6">
          <h2 id="add-ticket-modal-title" className="text-xl sm:text-2xl font-semibold text-hegira-navy">
            {modalTitle}
          </h2>
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-hegira-turquoise transition-colors"
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <input type="text" name="name" id="name" value={formData.name} onChange={handleInputChange} required
                     className={`w-full py-2.5 pl-3 pr-3 bg-white border rounded-lg shadow-sm focus:ring-1 focus:ring-hegira-turquoise transition-colors placeholder-gray-400 text-sm ${formErrors.name ? 'border-red-500' : 'border-gray-300'}`}
          ...
          <div>
            ...
            <div className="relative">
              <input type="text" name="categoryLabel" id="categoryLabel" value={formData.categoryLabel || ''} onChange={handleInputChange}
                     className={`w-full py-2.5 pl-3 pr-3 bg-white border rounded-lg shadow-sm focus:ring-1 focus:ring-hegira-turquoise transition-colors placeholder-gray-400 text-sm border-gray-300`}
            ...
            <label htmlFor="price" className="block text-sm font-medium text-gray-700 mb-1">Harga Tiket (IDR) <span className="text-red-500">*</span></label>
             <div className="relative">
              <input type="number" name="price" id="price" value={formData.price} onChange={handleInputChange} required min="0" step="1000"
                     className={`w-full py-2.5 pl-3 pr-3 bg-white border rounded-lg shadow-sm focus:ring-1 focus:ring-hegira-turquoise transition-colors placeholder-gray-400 text-sm ${formErrors.price ? 'border-red-500' : 'border-gray-300'}`}
            ...
            <label htmlFor="description" className="block text-sm font-medium text-gray-700 mb-1">Deskripsi Tiket (Opsional)</label>
             <div className="relative">
                <textarea name="description" id="description" value={formData.description} onChange={handleInputChange} rows={3}
                        className="w-full py-2.5 pl-3 pr-3 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-1 focus:ring-hegira-turquoise transition-colors placeholder-gray-400 text-sm"
            ...
            <label htmlFor="maxQuantity" className="block text-sm font-medium text-gray-700 mb-1">Jumlah Tiket Tersedia <span className="text-red-500">*</span></label>
             <div className="relative">
                <input type="number" name="maxQuantity" id="maxQuantity" value={formData.maxQuantity} onChange={handleInputChange} required min="1"
                    className={`w-full py-2.5 pl-3 pr-3 bg-white border rounded-lg shadow-sm focus:ring-1 focus:ring-hegira-turquoise transition-colors placeholder-gray-400 text-sm ${formErrors.maxQuantity ? 'border-red-500' : 'border-gray-300'}`}
          ...
          <div className="pt-4 border-t border-gray-100">
            <div className="flex items-center mb-3">
                <input type="checkbox" id="useEventSchedule" name="useEventSchedule" checked={formData.useEventSchedule} onChange={handleInputChange} className="h-4 w-4 text-hegira-turquoise border-gray-300 rounded focus:ring-hegira-turquoise"/>
                            ...
                            <div className="flex-grow">
                             ...
                             <label className="flex items-center space-x-1.5 text-xs text-gray-700 mt-1 md:mt-6 whitespace-nowrap">
                                <input type="checkbox" name="ticketIsTimeRange" checked={!formData.ticketIsTimeRange} 
                                       ...
                                       className="form-checkbox h-3.5 w-3.5 text-hegira-turquoise rounded focus:ring-hegira-turquoise/20"/>
                                <span>Selesai</span>
                            </label>
                        </div>
                    </div>
          ...
          <div className="flex flex-col sm:flex-row-reverse gap-3 pt-4 border-t border-gray-200">
            <button
              type="submit"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-lg border border-transparent shadow-sm px-6 py-2.5 bg-hegira-turquoise text-base font-medium text-white hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-hegira-yellow sm:text-sm transition-colors"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="w-full sm:w-auto inline-flex justify-center rounded-lg border border-gray-300 shadow-sm px-6 py-2.5 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-hegira-turquoise sm:text-sm transition-colors"
            >
              Batal
            </button>
          </div>
      ...
      <style>{`
        ...
        .custom-scrollbar-modal::-webkit-scrollbar-thumb { background: var(--hegira-chino, #d0cea9); border-radius: 10px; }
        ...
        .input-field-sm:focus { outline:2px solid transparent; outline-offset:2px; --tw-ring-offset-shadow: var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color); --tw-ring-shadow: var(--tw-ring-inset) 0 0 0 calc(1px + var(--tw-ring-offset-width)) var(--tw-ring-color); box-shadow: var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow, 0 0 #0000); border-color: var(--hegira-turquoise) !important; --tw-ring-color: rgba(75, 153, 142, 0.2) !important;}
