...
const SingleCrewBarcodeModal: React.FC<SingleCrewBarcodeModalProps> = ({ isOpen, onClose, crewMember }) => {
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="bg-white text-hegira-navy p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-sm relative transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear text-center">
        <button
          onClick={onClose}
        ...
        <h2 id="single-crew-barcode-modal-title" className="text-lg sm:text-xl font-semibold text-hegira-deep-navy mb-2">
        ...
        <div className="mt-8 flex justify-center">
          <button
            onClick={onClose}
            className="px-6 py-2.5 text-sm font-medium text-white bg-hegira-turquoise hover:bg-opacity-90 rounded-lg shadow-sm transition-colors"
          >
            Tutup
          </button>
        </div>
