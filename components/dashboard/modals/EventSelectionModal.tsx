...
const MinimalEventCard: React.FC<MinimalEventCardProps> = ({ event, onSelect }) => {
  ...
  return (
    <div
      ...
      className="bg-white rounded-lg border border-gray-200 hover:border-hegira-turquoise cursor-pointer transition-all duration-200 overflow-hidden shadow-sm hover:shadow-md"
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="p-3">
        <h4 className="text-sm font-semibold text-hegira-navy truncate" title={event.name}>{event.name}</h4>
...
const EventSelectionModal: React.FC<EventSelectionModalProps> = ({ isOpen, onClose, onEventSelect, events }) => {
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="bg-white text-hegira-navy p-5 sm:p-6 rounded-xl shadow-2xl w-full max-w-2xl relative transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center pb-3 border-b border-gray-200 mb-4">
          <h2 id="event-selection-modal-title" className="text-lg font-semibold text-hegira-deep-navy">
            Pilih Event
          </h2>
        ...
        <div className="mb-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-hegira-turquoise focus:border-hegira-turquoise"
            />
          </div>
        </div>
