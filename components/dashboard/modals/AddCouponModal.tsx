...
const AddCouponModal: React.FC<AddCouponModalProps> = ({ isOpen, onClose, onSave, initialCouponData, availableTickets }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      const defaultState: CouponFormData = {
      ...
      if (initialCouponData) {
        // Start with a complete default state and spread initial data over it.
        // This ensures all properties are defined only once and avoids linter errors.
        setFormData({
          ...defaultState,
          ...initialCouponData,
        });
      } else {
        // Use a fresh default state for a new coupon.
