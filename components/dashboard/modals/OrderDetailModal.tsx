...
const DetailItem: React.FC<{ label: string; value?: string | number; icon?: React.ElementType; className?: string }> = ({ label, value, icon: Icon, className }) => (
  <div className={`flex items-start ${className}`}>
    ...
    <span className="text-xs text-hegira-deep-navy font-medium break-words flex-grow">{value || '-'}</span>
  </div>
);
...
const OrderDetailModal: React.FC<OrderDetailModalProps> = ({ isOpen, onClose, order, fullEventData, onNavigate }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white text-hegira-navy p-5 sm:p-6 md:p-8 rounded-xl shadow-2xl w-full max-w-xl lg:max-w-2xl relative transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center pb-4 border-b border-gray-200 mb-4">
          <h2 id="order-detail-modal-title" className="text-lg sm:text-xl font-semibold text-hegira-deep-navy flex items-center">
            <ShoppingCart size={22} className="mr-2.5 text-hegira-turquoise" /> Detail Pesanan #{order.id}
          </h2>
          <button onClick={onClose} className="text-gray-400 hover:text-hegira-turquoise transition-colors" aria-label="Tutup modal">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="overflow-y-auto custom-scrollbar-modal flex-grow pr-1">
          ...
          <section className="mb-4">
            <h3 className="text-md font-semibold text-hegira-navy mb-2">Detail Event</h3>
          ...
          {/* Booker Details */}
          <section className="mb-4">
            <h3 className="text-md font-semibold text-hegira-navy mb-2">Detail Pemesan</h3>
          ...
          {/* Ticket Details */}
          <section className="mb-4">
            <h3 className="text-md font-semibold text-hegira-navy mb-2">Rincian Tiket</h3>
          ...
          {(order.coupon || order.originalPrice !== order.totalPrice) && (
            <section className="mb-4">
              <h3 className="text-md font-semibold text-hegira-navy mb-2">Diskon & Total</h3>
           ...
           <div className="mt-3 pt-3 border-t flex justify-between items-center">
                <span className="text-md font-bold text-hegira-deep-navy">Total Pembayaran</span>
                <span className="text-xl font-bold text-hegira-yellow">{formatCurrency(order.totalPrice)}</span>
            </div>
          ...
          {order.additionalTicketHolders && order.additionalTicketHolders.length > 0 && (
            <section className="mt-4 pt-4 border-t">
              <h3 className="text-md font-semibold text-hegira-navy mb-2">Data Pemegang Tiket Tambahan</h3>
        ...
        <div className="mt-6 pt-4 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-3">
          <button
            onClick={onClose}
            className="w-full sm:w-auto justify-center inline-flex items-center px-6 py-2.5 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-hegira-turquoise transition-colors"
          >
            Tutup
          </button>
          {order.tickets && order.tickets.length > 0 && fullEventData && (
             <button
                ...
                className="w-full sm:w-auto justify-center inline-flex items-center gap-2 px-6 py-2.5 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-hegira-turquoise hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-hegira-yellow transition-colors"
             >
