...
const DashboardTopBar: React.FC<DashboardTopBarProps> = ({
  ...
  switch (userRole) {
    case 'visitor':
      dynamicRoleButtonStyles = `bg-hegira-turquoise/10 text-hegira-turquoise border border-hegira-turquoise hover:bg-hegira-turquoise/20 ${baseDashboardButtonStyles}`;
      break;
    case 'creator':
      dynamicRoleButtonStyles = `bg-hegira-yellow/10 text-hegira-navy border border-hegira-yellow hover:bg-hegira-yellow/20 ${baseDashboardButtonStyles}`;
      break;
    case 'organization':
      dynamicRoleButtonStyles = `bg-hegira-chino/10 text-hegira-navy border border-hegira-chino hover:bg-hegira-chino/20 ${baseDashboardButtonStyles}`;
      break;
    default: 
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            <button
              ...
              className="text-gray-500 hover:text-hegira-turquoise focus:outline-none focus:ring-2 focus:ring-inset focus:ring-hegira-yellow p-2 rounded-md -ml-2 md:hidden"
              ...
              <ol className="flex items-center space-x-1 text-sm">
                {breadcrumbSegments.map((segment, index) => (
                  <li key={index} className="flex items-center">
                    ...
                    {segment.isCurrent ? (
                      <span className="font-medium text-hegira-navy flex items-center">
                      ...
                      <button
                        ...
                        className="text-gray-500 hover:text-hegira-turquoise transition-colors flex items-center"
                      >
              ...
              <ol className="flex items-center space-x-1 text-xs truncate">
                {breadcrumbSegments.map((segment, index) => (
                  <li key={`mobile-${index}`} className="flex items-center truncate">
                    ...
                    {segment.isCurrent ? (
                      <span className="font-medium text-hegira-navy truncate flex items-center">
                      ...
                      <button
                        onClick={() => segment.viewId && onSelectView(segment.viewId, segment.data)}
                        className="text-gray-500 hover:text-hegira-turquoise transition-colors truncate flex items-center"
                      >
