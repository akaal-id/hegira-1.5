...
const Footer: React.FC<FooterProps> = ({ onNavigate, currentPage }) => {
  ...
  const footerBaseClasses = "bg-hegira-turquoise text-hegira-deep-navy pt-16 pb-8";
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="md:grid md:grid-cols-4 md:gap-x-8 lg:gap-x-10 mb-12">
          ...
          <div className="space-y-4 mb-10 md:mb-0 md:col-span-1">
            ...
            >
              <Logo className="h-10 w-auto text-hegira-white" useGradient={true} />
            </button>
            <p className="text-sm leading-relaxed text-hegira-white">
            ...
            <div className="flex space-x-4 mt-4">
              {socialMedia.map(social => (
                <a 
                  ...
                  className="text-hegira-white hover:text-hegira-yellow transition-colors duration-300 transform hover:scale-110"
                >
                  <social.icon size={22} />
                </a>
              ))}
          ...
          */}
          <div className="grid grid-cols-2 gap-x-8 gap-y-10 md:col-span-3 md:grid-cols-3">
            {/* Quick Links */}
            <div>
              <h5 className="text-lg font-semibold font-jakarta text-hegira-yellow mb-5">Navigasi</h5>
              <ul className="space-y-3">
                {quickLinks.map(link => (
                  <li key={link.name}>
                    <button 
                      onClick={() => link.target && onNavigate(link.target)} 
                      className="text-sm text-hegira-white hover:underline transition-colors duration-300"
                    >
                      {link.name}
                    </button>
                  </li>
            ...
            {/* Legal & Support */}
            <div>
              <h5 className="text-lg font-semibold font-jakarta text-hegira-yellow mb-5">Legal & Bantuan</h5>
              <ul className="space-y-3">
                {legalLinks.map(link => (
                  <li key={link.name}>
                    <a href={link.href} className="text-sm text-hegira-white hover:underline transition-colors duration-300">
                      {link.name}
                    </a>
                  </li>
                ))}
            ...
            {/* Contact Info */}
            <div>
              <h5 className="text-lg font-semibold font-jakarta text-hegira-yellow mb-5">Hubungi Kami</h5>
              <ul className="space-y-3 text-sm text-hegira-white">
                <li className="flex items-start">
                  <Mail size={18} className="mr-3 mt-1 text-hegira-white flex-shrink-0" />
                  <a href="mailto:asiakaryalumina@gmail.com" className="hover:underline transition-colors">asiakaryalumina@gmail.com</a>
                </li>
                <li className="flex items-start">
                  <Phone size={18} className="mr-3 mt-1 text-hegira-white flex-shrink-0" />
                  <a href="tel:+622112345678" className="hover:underline transition-colors">+62 21 1234 5678</a>
                </li>
                <li className="flex items-start">
                  <MapPin size={18} className="mr-3 mt-1 text-hegira-white flex-shrink-0" />
        ...
        <div className="border-t border-hegira-white/30 pt-8 mt-8 text-center text-sm text-hegira-white">
