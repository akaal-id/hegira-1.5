...
const CompanyReviews: React.FC<CompanyReviewsProps> = ({ reviews }) => {
  if (!reviews || reviews.length === 0) {
     return (
        <section aria-labelledby="reviews-title" className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            <h2 id="reviews-title" className="text-xl font-semibold text-hegira-deep-navy mb-4">Ulasan Klien</h2>
  ...
  return (
    <section aria-labelledby="reviews-title" className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
      <h2 id="reviews-title" className="text-xl font-semibold text-hegira-deep-navy mb-5">
          ...
          <div key={index} className="border-b border-gray-100 pb-4 last:border-b-0 last:pb-0">
            <div className="flex items-start sm:items-center justify-between mb-1.5 flex-wrap gap-y-1">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-sm font-semibold text-hegira-deep-navy">{review.author}</p>
              ...
              <div className="flex items-center">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    size={15}
                    className={i < review.rating ? 'text-hegira-yellow' : 'text-gray-300'}
