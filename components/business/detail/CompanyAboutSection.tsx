...
const CompanyAboutSection: React.FC<CompanyAboutSectionProps> = ({ description, specializations }) => {
  return (
    <section aria-labelledby="about-company-title" className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
      <h2 id="about-company-title" className="text-xl font-semibold text-hegira-deep-navy mb-3">
        Tentang Perusahaan
      </h2>
      <div 
      ...
      {specializations && specializations.length > 0 && (
        <div>
          <h3 className="text-md font-semibold text-hegira-deep-navy mb-2.5">Spesialisasi & Keunggulan:</h3>
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5">
            {specializations.map((spec, index) => (
              <li key={index} className="flex items-center text-sm text-gray-700">
                <CheckCircle size={15} className="text-hegira-turquoise mr-1.5 flex-shrink-0" />
                {spec}
              </li>
            ))}
          </ul>
