        ...
        <div className="relative -mt-16 md:-mt-20 lg:-mt-24">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 md:gap-6 pt-4">
            ...
            <div className="pb-1 text-center sm:text-left">
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-hegira-deep-navy bg-white/80 backdrop-blur-sm px-3 py-1 rounded-md shadow-sm inline-block">{companyName}</h1>
              {rating !== undefined && (
                <div className="flex items-center mt-1.5 justify-center sm:justify-start bg-white/80 backdrop-blur-sm px-2 py-0.5 rounded-md shadow-sm max-w-fit mx-auto sm:mx-0">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      size={16}
                      className={i < Math.round(rating) ? 'text-hegira-yellow' : 'text-gray-300'}
                      fill={i < Math.round(rating) ? 'currentColor' : 'none'}
                    />
                  ))}
                  <span className="ml-1.5 text-xs text-hegira-deep-navy font-medium">({rating.toFixed(1)} dari 5)</span>
                </div>
              )}
            </div>
          </div>
