    ...
    <section aria-labelledby="company-metrics-title" className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
        {metrics.map((metric, index) => {
          ...
          return (
            <div key={index} className="bg-gradient-to-br from-hegira-turquoise/10 via-hegira-turquoise/5 to-white p-4 rounded-lg border border-hegira-turquoise/20 text-center transition-all duration-300 hover:shadow-md hover:border-hegira-turquoise/30">
              <IconComponent size={26} className="text-hegira-turquoise mx-auto mb-2" />
              <p className="text-xl sm:text-2xl font-bold text-hegira-deep-navy">{metric.value}</p>
