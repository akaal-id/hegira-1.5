...
const MeetingSchedulerModal: React.FC<MeetingSchedulerModalProps> = ({
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="bg-white text-hegira-deep-navy p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-lg relative transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear max-h-[90vh] flex flex-col">
        <button
          onClick={onClose}
        ...
        <h2 id="meeting-scheduler-modal-title" className="text-xl sm:text-2xl font-semibold text-hegira-navy mb-2 flex items-center">
          <Calendar size={24} className="mr-3 text-hegira-turquoise" />
          Jadwalkan Meeting Dengan
        </h2>
        <p className="text-lg font-medium text-hegira-turquoise mb-6">{vendorName}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 overflow-y-auto custom-scrollbar-modal pr-1 flex-grow">
          <div>
            ...
            <select
              ...
              className="w-full py-2.5 px-3 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-1 focus:ring-hegira-turquoise transition-colors text-sm appearance-none"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select id="meetingDuration" value={meetingDuration} onChange={(e) => setMeetingDuration(e.target.value)} className="w-full py-2.5 px-3 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-1 focus:ring-hegira-turquoise transition-colors text-sm appearance-none">
            ...
            <div>
              ...
              <select id="meetingType" value={meetingType} onChange={(e) => setMeetingType(e.target.value)} className="w-full py-2.5 px-3 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-1 focus:ring-hegira-turquoise transition-colors text-sm appearance-none">
          ...
          <div>
            ...
            <textarea
              ...
              required
              className="w-full py-2 px-3 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-1 focus:ring-hegira-turquoise transition-colors text-sm placeholder-gray-400"
              placeholder="Contoh: Diskusi potensi kerjasama proyek X, Presentasi produk Y..."
            />
          </div>
          <div className="p-3 bg-blue-50 border-l-4 border-hegira-turquoise rounded-md text-xs text-blue-700">
        ...
        <div className="mt-6 pt-4 border-t border-gray-200 flex flex-col sm:flex-row-reverse gap-3">
          <button
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-lg border border-transparent shadow-sm px-6 py-2.5 bg-hegira-yellow text-hegira-navy text-sm font-bold hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-hegira-navy transition-colors"
          >
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-full sm:w-auto inline-flex justify-center rounded-lg border border-gray-300 shadow-sm px-6 py-2.5 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-hegira-turquoise transition-colors"
          >
            Batal
          </button>
        </div>
