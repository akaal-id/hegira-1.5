...
const EventCard: React.FC<EventCardProps> = (props) => {
  ...
  const categoryStyles = {
    B2C: 'bg-hegira-turquoise text-hegira-deep-navy text-xs',
    B2B: 'bg-hegira-navy text-hegira-white text-xs',
    B2G: 'bg-hegira-yellow text-hegira-deep-navy text-xs',
  };
  ...
  let finalPriceColorClass = 'text-hegira-yellow';
  ...
  if (numericPrices && numericPrices.length > 0) {
    ...
    if (!props.displayPrice.toLowerCase().startsWith('rp')) {
      finalPriceColorClass = 'text-hegira-deep-navy'; 
    }
  }


  return (
    <div className="bg-hegira-card-bg rounded-xl border border-hegira-navy/10 hover:border-hegira-turquoise/20 overflow-hidden transition-all duration-300 group flex flex-col event-card h-full">
      <div className="relative w-full overflow-hidden" style={{ paddingTop: '37.5%' /* 6 / 16 = 0.375 */ }}>
        ...
        <button
          ...
          className="absolute top-4 right-4 p-2.5 bg-white/80 backdrop-blur-sm rounded-full text-hegira-yellow hover:bg-white/100 transition-colors duration-200 shadow-md"
      ...
      <div className="p-5 flex flex-col flex-grow">
        <h3 className="text-xl font-bold font-jakarta text-hegira-deep-navy mb-2 leading-tight group-hover:text-hegira-gradient-start transition-colors truncate">{name}</h3>
        
        <div className="flex items-center text-sm text-gray-500 mb-1">
          <CalendarDays size={15} className="mr-1.5 text-hegira-turquoise flex-shrink-0" />
          <span>{dateDisplay}</span>
        </div>
        <div className="flex items-center text-sm text-gray-500 mb-2">
          <Clock size={15} className="mr-1.5 text-hegira-turquoise flex-shrink-0" />
          <span>{formatEventTime(timeDisplay, timezone)}</span>
        </div>
        <div className="flex items-center text-sm text-gray-500 mb-3"> 
          <MapPin size={15} className="mr-1.5 text-hegira-turquoise flex-shrink-0" />
          <span>{location}</span>
        </div>
        ...
        <div className="mt-auto"> {/* Pushes content below to the bottom */}
          ...
          <button 
            ...
            className="w-full bg-gradient-to-r from-hegira-turquoise to-hegira-turquoise/80 text-white font-semibold py-3 px-4 rounded-lg
                       transition-all duration-300
                       flex items-center justify-center group/button
                       hover:bg-gradient-to-r hover:from-hegira-turquoise/30 hover:to-hegira-turquoise/40 hover:text-hegira-turquoise"
