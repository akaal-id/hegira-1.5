      ...
      <div className="relative bg-hegira-white w-full max-w-lg rounded-2xl shadow-2xl border border-hegira-navy/10 overflow-hidden transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear">
        ...
        <div className="p-6 sm:p-8">
          <div className="text-center mb-6">
            ...
            <h1 id="role-switch-modal-title" className="text-xl sm:text-2xl font-semibold text-hegira-navy">Ganti Peran Pengguna</h1>
            <p className="text-xs text-gray-500 mt-1">
              Peran Anda saat ini: <strong className="text-hegira-turquoise">{getRoleDisplayName(currentUserRole)}</strong>
            </p>
          </div>
              ...
              <button
                ...
                className={`w-full flex items-center justify-between text-left p-3.5 border rounded-lg transition-all duration-200
                           ${selectedRole === roleItem.internal 
                             ? 'border-hegira-turquoise bg-hegira-turquoise/10 ring-2 ring-hegira-turquoise/70' 
                             : 'border-gray-300 hover:border-hegira-turquoise/50 hover:bg-gray-50'}
                           ${currentUserRole === roleItem.internal 
                             ? 'opacity-70 cursor-not-allowed bg-gray-100' 
                             : 'focus:outline-none focus:ring-2 focus:ring-hegira-yellow focus:ring-offset-1'}`}
                aria-pressed={selectedRole === roleItem.internal}
              >
                <div className="flex items-start">
                  <roleItem.icon className={`h-5 w-5 mr-3 mt-0.5 flex-shrink-0 ${selectedRole === roleItem.internal || currentUserRole === roleItem.internal ? 'text-hegira-turquoise' : 'text-gray-500'}`} />
                  <div>
                    <span className={`text-sm font-medium ${selectedRole === roleItem.internal || currentUserRole === roleItem.internal ? 'text-hegira-turquoise' : 'text-hegira-navy'}`}>
                      {roleItem.display}
                ...
                {selectedRole === roleItem.internal && currentUserRole !== roleItem.internal && <CheckCircle size={20} className="text-hegira-turquoise flex-shrink-0" />}
              </button>
            ))}
          </div>
          ...
          <div className="mt-8 flex flex-col sm:flex-row-reverse gap-3">
            <button
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-lg border border-transparent shadow-sm px-6 py-2.5 bg-hegira-yellow text-hegira-navy text-sm font-bold hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-hegira-navy transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
